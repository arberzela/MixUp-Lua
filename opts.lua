...
function M.parse(arg)
   ...
   ------------- Training options --------------------
   cmd:option('-algorithmType',  'SGD',    'Options: SGD | SGDdec | SGDW | ADAM | ADAMdec | ADAMW')
   cmd:option('-LRdec',          'true',   'Whether to decay LR or not')
   cmd:option('-Te',              1800,    'Initial period of restarts: 1 | 2 | 3 | 4 | ...', 'number')
   ...
   cmd:option('-snapshots',            'snapshots', 'Directory in which to save snapshots')
   ...
   -------- Cutout options ----------
   cmd:option('-cutout_half_size', 8, 'Half of the cutout patch size')
   ------ Mixup options ------
   cmd:option('-alpha',         0.2,      'the parameter defining B(alpha, alpha) where lam is sampled from')
   -------------------------
   cmd:text()
