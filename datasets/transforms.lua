...
function M.CutOut(half_length)
	return function(input, seed)
		...
		local x = torch.random(1, w)
		torch.manualSeed(seed + 50001)
		local y = torch.random(1, h)
...
function M.RandomCrop(size, padding)
   ...
   return function(input, seed)
      ...
      torch.manualSeed(seed + 100001)
      local x1 = torch.random(0, w - size)
      torch.manualSeed(seed + 150001)
      local y1 = torch.random(0, h - size)
   ...
   return function(input, seed)
      ...
      repeat
         ...
         if h <= input:size(2) and w <= input:size(3) then
            torch.manualSeed(seed + 250001)
            local y1 = torch.random(0, input:size(2) - h)
            torch.manualSeed(seed + 300001)
